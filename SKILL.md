...
## 每条强制字段（不可省略）
- 标题（完整标题，不缩写）
- 栏目名（必须是播客栏目/频道/source 名，例如 Lenny's Podcast、a16z Podcast、Y Combinator、Moonshots；不是“一周榜 Top 3 / 三周榜 Top 8”这类榜单名。展示时放在标题后面，例如 `标题｜Lenny's Podcast`）
- 嘉宾（必须写为：姓名｜公司｜Title）
- 发布时间（具体日期）
- 播放量
...
4) 每条是否包含全部强制字段（尤其“姓名｜公司｜Title”，以及标题后面的真实播客栏目/source 名）；
